"""
tests/test_execution_sms.py - Endpoint tests for /execute SMS behavior.

Execution-path tests mock Supabase and SMS sending so no external services are used.
"""
...
def test_execute_sms_notify_with_valid_phone(client):
    fake_supabase = FakeSupabase([
        {"id": "contact-1", "phone": "0400000000"},
    ])
    ...
    with patch("services.execution_service.supabase", fake_supabase):
        with patch("services.execution_service.send_sms", return_value={
            "success": True,
            "message_id": "msg-1",
            "error": None,
        }) as mock_send_sms:
    ...
    assert body["results"][0]["message"] == "SMS sent to 1 contact."
    mock_send_sms.assert_called_once_with("0400000000", "Crew update")
...
def test_execute_sms_notify_with_missing_phone(client):
    fake_supabase = FakeSupabase([
        {"id": "contact-2", "phone": None},
    ])
    ...
    with patch("services.execution_service.supabase", fake_supabase):
        with patch("services.execution_service.send_sms") as mock_send_sms:
            response = client.post("/execute", json=payload)

    assert response.status_code == 200
    body = response.json()
    assert body["results"][0]["success"] is False
    assert body["results"][0]["message"] == "No valid phone numbers found for SMS delivery."
...
def test_execute_non_sms_notify_unchanged(client):
    fake_supabase = FakeSupabase([
        {"id": "contact-3", "phone": "0400000001"},
    ])
